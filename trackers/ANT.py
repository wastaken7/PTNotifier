...
class ANT(BaseTracker):
    ...
    def __init__(self, cookie_path: Path):
        super().__init__(cookie_path, "ANT", "https://anthelion.me/")
    ...
    async def fetch_notifications(send_telegram: Callable[[dict[str, Any], str, str], Coroutine[Any, Any, None]]):
        """Module-level entry point for Anthelion sites."""
        cookie_files = glob.glob(str(Path("./cookies") / "OTHER" / "ANT.txt"))
        if not cookie_files:
            return

        sessions = [ANT(Path(f)) for f in cookie_files]
