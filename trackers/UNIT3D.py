...
class UNIT3D(BaseTracker):
    ...
    async def initialize(self):
        ...
        if not soup:
            console.print(f"{self.domain}: Initialization failed.")
            return
    ...
    async def _mark_as_read(self, item: dict) -> bool:
        ...
        try:
            ...
            console.print(f"{self.domain}: Exception marking as read: {e}")
            return False

    async def process(self, send_telegram: Callable):
        try:
            ...
            for item in notifs + msgs:
                if not self.first_run:
                    await send_telegram(item, self.domain, item["url"])
                    await asyncio.sleep(3)
                if await self._mark_as_read(item):
                    self._ack_item(item["id"])

        except Exception as e:
            console.print(f"{self.domain}: Error processing {self.domain}: {e}")
        finally:
            await self.client.aclose()

    @staticmethod
    async def fetch_notifications(send_telegram: Callable[[dict[str, str], str, str], Coroutine[Any, Any, None]]):
        cookie_files = glob.glob(str(Path("./cookies") / "UNIT3D" / "*.txt"))
        if not cookie_files:
            return

        sessions = [UNIT3D(Path(f)) for f in cookie_files]
