...
class AVISTAZ(BaseTracker):
    ...
    async def process(
        ...
        try:
            ...
            console.print(f"{self.domain}: Error processing session for {self.domain}: {e}")
        finally:
    ...
    async def fetch_notifications(send_telegram: Callable[[dict[str, str], str, str], Coroutine[Any, Any, None]]):
        cookie_files = glob.glob(str(Path("./cookies") / "AVISTAZ" / "*.txt"))
        if not cookie_files:
            return

        sessions = [AVISTAZ(Path(f)) for f in cookie_files]
